...
const users = [
  { name: 'Hedy Lamarr', imageUrl: 'https://media.kasperskydaily.com/wp-content/uploads/sites/92/2015/11/06023739/hedy-lamarr-FB.jpg', imageSize: 90 },
  { name: 'Ada Lovelace', imageUrl: 'https://resh.com.br/wp-content/uploads/2023/03/Ada-Lovelace.jpg', imageSize: 90 },
...
function ShoppingList() {
  return (
    <ul>
      {products.map(p => <li key={p.id} style={{ color: p.isFruit ? 'magenta' : 'darkgreen' }}>{p.title}</li>)}
    </ul>
  );
}
...
function App() {
  ...
  function handleLogin(e) {
    e.preventDefault();
    if (username === 'admin' && password === 'adminpass') { setRole('admin'); setIsLoggedIn(true); }
    else if (username && password) { setRole('user'); setIsLoggedIn(true); }
    else { alert('Identifiants invalides'); }
  }

  function handleLogout() { setIsLoggedIn(false); setRole(null); setUsername(''); setPassword(''); }

  if (!isLoggedIn) {
    return (
      <form onSubmit={handleLogin}>
        <h2>Connexion</h2>
        <input type="text" placeholder="Nom d'utilisateur" value={username} onChange={e => setUsername(e.target.value)} required /><br/>
